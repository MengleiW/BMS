...
def abc_smc_estimation(method, k, t_window, y_obs, yp_obs,
    ...
    for t in range(num_generations):
        ...
        if t == 0:
            ...
            weights[t, :acc] = temp_weights / weight_sum if weight_sum > 0 else 1.0 / acc
    ...
    g_std = np.sqrt(np.average((final_particles - g_mean)**2, weights=final_weights)) if actual_particles > 1 else 0.1
    ...
    return g_mean, g_std, evidence, final_particles, final_weights
...
def run_simple_analysis():
    ...
    for i in range(n_windows):
        ...
        g_eu_abc, _, Z_eu_abc, final_particles_eu, final_weights_eu = res_eu
        ...
        g_tr_abc, _, Z_tr_abc, final_particles_tr, final_weights_tr = res_tr
