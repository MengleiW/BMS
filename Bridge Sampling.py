...
def OptimalBridge (method, data,N,N1,N2, check_points,gammas, observed_y, observed_yp, sigma_y, sigma_yp,Timepoint_of_interest):
    ...
    Z = []
    Zhat = Slove_Z(data,check_points,gammas, observed_y, observed_yp, sigma_y, sigma_yp)[Timepoint_of_interest]
    a,target_function1=  proposal_l(method, gammas, initial_conditions, T, k)
    proposal_function1  = lambda x: np.random.multivariate_normal(x, cov=np.eye(len(x)) *0.3)
    target_function2 = lambda x : scipy.stats.multivariate_normal.logpdf(x, cov=np.eye(len(x)) * 0.3)
    proposal_function2  = lambda x: np.random.multivariate_normal(x, cov=np.eye(len(x)) * 0.3)
    for i in range (N-1):
         #Taking sampling using Metropolis Hasting algrithm. 
         tht1 = Metropolis_hasting(N,target_function1,proposal_function1 )
         tht2 = Metropolis_hasting(N,target_function2,proposal_function2 )
         
         
         #Finding Q11
         q11 =  proposal_l(method, tht1, initial_conditions, T, k)
         #print('l1=',likelihood1)
         
         
         #Finding Q12
         q12 = proposal_l(method, tht2, initial_conditions, T, k)
         #print('q12=',q12)
         ...
         Q1 = np.logaddexp.reduce(np.log(q12)) - np.logaddexp.reduce(np.log(N1*q12 + N2*Zhat*q22))+N1
         Q2 = np.logaddexp.reduce(np.log(q21)) - np.logaddexp.reduce(np.log(N1*q11 + N2*Zhat*q21))+N2
         print('Q1=',Q1)
         print('Q2=',Q2)
...
if __name__ == '__main__':   
    ...
    Z_e = OptimalBridge (euler_forward, data_e,N,N1,N2, check_points,gammas, observed_y, observed_yp, sigma_y, sigma_yp,Timepoint_of_interest)
    Z_t = OptimalBridge (trapezoidal_method,data_t,N,N1,N2, check_points,gammas, observed_y, observed_yp, sigma_y, sigma_yp,Timepoint_of_interest)
